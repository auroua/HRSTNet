...
def build_dynunet(cfgs):
    model = DynUNet(
        cfgs.MODEL.DYNUNET.DIM,
        cfgs.MODEL.IN_CHANNEL,
        cfgs.MODEL.OUT_CHANNEL,
        cfgs.MODEL.DYNUNET.KERNELS,
        cfgs.MODEL.DYNUNET.STRIDES,
        cfgs.MODEL.DYNUNET.STRIDES[1:],
        filters=cfgs.MODEL.DYNUNET.FILTERS,
        norm_name=(cfgs.MODEL.NORM.upper(), {"affine": True}),
        act_name=("leakyrelu", {"inplace": False, "negative_slope": 0.01}),
        deep_supervision=cfgs.MODEL.DYNUNET.DEEP_SUPERVISION,
        deep_supr_num=cfgs.MODEL.DYNUNET.DEEP_SUPR_NUM,
        res_block=cfgs.MODEL.DYNUNET.RES_BLOCK,
        trans_bias=True,
    )
    return model
