...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 encoder_scale=1, axial_attention=False, heads=8, dim_heads=32, volume_shape=(128, 128, 128),
                 no_attention=[0], dropout_level=[4]):  # extra
        """
...
class AxialAttention3D(nn.Module):
    def __init__(self, n_features, emb_shape, heads, dim_heads, non_lin, non_lin_kwargs, bn=True, sum_axial_out=True,
...
def build_extending_nnunet(cfgs):
    conv_op = torch.nn.Conv3d
    dropout_op = torch.nn.Dropout3d
    norm_op = torch.nn.GroupNorm
    net_nonlin = torch.nn.LeakyReLU

    NORM_OP_KWARGS = {'num_groups': 32, 'eps': 1e-05, 'affine': True}
    DROPOUT_OP_KWARGS = {'p': 0, 'inplace': True}
    NET_NOLIN_KWARGS = {'negative_slope': 0.01, 'inplace': True}


    network = Generic_UNet(
        cfgs.MODEL.IN_CHANNEL,
        cfgs.MODEL.EXTENDING_NN_UNET.BASE_NUM_FEATURES,
        cfgs.MODEL.OUT_CHANNEL,
        len(cfgs.MODEL.EXTENDING_NN_UNET.NET_NUM_POOL_OP_KERNEL_SIZES),
        cfgs.MODEL.EXTENDING_NN_UNET.CONV_PER_STAGE,
        2,
        conv_op,
        norm_op,
        NORM_OP_KWARGS,
        ...
        cfgs.MODEL.EXTENDING_NN_UNET.NET_NUM_POOL_OP_KERNEL_SIZES,
        cfgs.MODEL.EXTENDING_NN_UNET.NET_CONV_KERNEL_SIZES,
        False,
        True,
        True,
        512,
