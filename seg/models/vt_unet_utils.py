...
from seg.models.swin.swin_utils import window_partition, window_reverse, get_window_size, compute_mask
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
                 ...
                 act_layer="GELU", norm_layer=nn.LayerNorm, use_checkpoint=False):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class SwinTransformerSys3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.logger.info(
            "SwinTransformerSys3D expand initial----depths:{} \n depths_decoder:{}\n drop_path_rate:{}\n "
            "num_classes:{}\n embed_dims:{}\n window:{}".format(
                depths, depths_decoder, drop_path_rate, num_classes, embed_dim, window_size))
    ...
    def forward(self, x):
        x, x_downsample, v_values_1, k_values_1, q_values_1, \
        v_values_2, k_values_2, q_values_2 = self.forward_features(x)
        x = self.forward_up_features(x, x_downsample, v_values_1, k_values_1, q_values_1,
                                     v_values_2, k_values_2, q_values_2)
        x = self.up_x4(x)
        return x
